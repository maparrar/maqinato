...
<!-- MODAL GENERATOR -->
    <div id="check-out">
        <h2 id="title">Your cause mix</h2>
        <div class="section">
            <span class="amount checkoutText">amount</span>
            <span class="total">$ <span id="total_amount">25</span> <span></span></span>
            <span>
                <input type="checkbox" name="hide-amount" value="hide-amount" id="hide-amount" /> 
                <label for="hide-amount">share amount</label>
            </span>
        ...
        <div id="folio" class="section">
            <h5 class="checkoutText">create Folio</h5>
            <span class="createFolio">
                <input id="createFolio" type="checkbox"/>
                <input id="createFolioName" placeholder="Folio's name"/>
            </span>
        </div>
        <img class="sslGodaddy" src="<?php echo Router::rel("img")?>misc/GoDaddy-SSL-Cert.jpg"/>
                <?php
                    ...
                    if($balance==0){$disable='disabled="disable"';}
                        echo '<div id="balance">';
                            echo '<div id="amount"><img src="'.Router::rel("img").'icons/balance.png"/><span>$'.number_format($balance,2).'</span></div>';
...
<!-- Modal que se muestra cuando es la edición de un folio-->
<div id="modalSaveCombination">
<!-- MODAL GENERATOR -->
    <div id="saveCombination">
        <h2 id="title">Save the combination for <b>
        <?php 
            if(@$folio){
                echo $folio->getName(); 
            }
        ?></b> folio?</h2>
        <div id="giving" class="section checkout-share">
            <div id="submit" class="col last">
                    <a id="condfirmSave" href="" class="makeGiving">submit</a>
                <input id="checkoutData" type="hidden"/>
            </div>
        </div>
    </div>
...
<!-- MODAL GENERATOR -->
    <div id="creditCard" class="modal">
        ...
        <?php
            if($user->getId()===1){
                echo('<a id="withoutPay" href="">Without pay</a>');
            }
        ?>
        <img id="loading" src="">
        </div>
