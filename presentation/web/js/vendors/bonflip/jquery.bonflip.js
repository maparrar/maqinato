...
/*maqinato (www.https://github.com/maparrar/maqinato) - Bonflip v.0.4
 *Jan 2013
...
;(function($){
    ...
    function init(obj,optUser,callback){
...
//            if(!elem.trigger){
//                elem.trigger=elem.closest(".activity").find(".btn-flip")
//            }
...
//            });
    ...
    //Flip function
    function flip(selector,callback){
        var time=800; //milliseconds
        var newFace=parseInt(selector.attr("face"))+1;
        var countLi=selector.children("li").length;
        if(newFace>countLi){
            newFace=1;
        }
        if(countLi>1){
            selector.closest(".activity, .box").find(".btn-flip").toggleClass("active");
            selector.animate(
                {
