...
;(function($){
    ...
    function parse_link(obj,opts,callback){
        ...
        if (!isValidURL(url)) {
            ...
            $.ajax({
                type: "POST",
                url: opts.fetchScript,
                data: {url:escape(url)},
                timeout: 10000, // in milliseconds
                success: function(response) {
                    //Set Content
                    if(!response.title||response.title==="Bad Request"){
                        response.title="Type your title";
                    }
                    if(!response.description||response.title==="Bad Request"){
                        response.description="Type your description";
                    }
                    ...
                    if(response.total_images>0){
                        ...
                        respImages.empty();
                        respImages.css({"background":"none","border":"none"});
                        ...
                        respImages.css("background","#bbb");
                        respImages.height(70);
                ...
                error: function(request, status, err) {
                    if(status === "timeout") {
                        system.message("The story could not be read");
                    }
                }
            });
        }
