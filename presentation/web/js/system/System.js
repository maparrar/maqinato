 ...
 * TODO: Make pseudo class for each aspect. i.e. Miniprofile, Daemons, Search, ...
 **/
function System(){
    "use strict";
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    var obj=this;
    obj.application="maqinato";     //Detect the application name
    ...
    obj.ajaxDonation=null;          //Ajax class to load giving data from server
    obj.ajaxNonprofits=null;        //Ajax class to load giving data from server
    obj.ajaxRecoveryPassword=null;        //Ajax class to load giving data from server
    ...
    obj.init=function(options){
        ...
        if(obj.options.giving&&obj.config.user){
            ...
            obj.giving.init({
                ...
                amountMin:obj.config.maqinato.amountMin,
                amountDefault:obj.config.maqinato.amountDefault,
                amountIncrement:obj.config.maqinato.amountIncrement,
       ...
       $("#cityinput").focus(function(){ 
            if($("#country").val()==""){
                $("#country").focus();
                $("#cityinput").addClass("errorSignup").val("select a country please");
            }
       });

        $("#cityinput").focusout(function(){ 
            //$(".user-registry #content").hide(),100);
            if($(".user-registry #content #list p").size()==1){
                $("#cityinput").val($(".user-registry #content #list p").text());
                $("#cityinput").attr("idCity",$(".user-registry #content #list p").attr("value"));
                $(".user-registry #content").hide("slow");
            }else{
                $(".user-registry #content").hide("slow");
            }
       ...
       $("#cityinput").keyup(function(){
            var city = system.security.secureString($("#cityinput").val());
            var country = $("#country").val();
            system.ajax.readCity(country,city,function(response){
                $(".user-registry #content #list p").remove();
                $(".user-registry #content #list").append(response.listCities); 
                $(".user-registry #content").find("p").click(function(){
                    $("#cityinput").val($(this).text()).attr("idCity",$(this).attr("value"));
                    $(".user-registry #content").hide("slow");
                });
            });
            if($(".user-registry #content #list p").size()<1){
                $("#cityinput").attr("idCity","");
                $(".user-registry #content").hide("slow");
            }else{
                $(".user-registry #content").show();
            }
        });
        //()
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> SYSTEM <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    //Función para invitar a amigos fuera de maqinato (que se haya verificado que no existen)
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> UTILS <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
 ...
 //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> FACEBOOK <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    obj.initFacebook=function(callback){
